...
parser.add_argument('--shot', default='single', type=str, help='single or multiple shot')
...
#parser.add_argument('--mask_layer', nargs='+', default=[], help='mask layer')
#parser.add_argument('--conv_mode', nargs='+', default=[], help='conv_mode')
...
if dataset == 'sysu':
    ...
    if shot == 'single':
        ...
        query_img, query_label, query_cam = process_query_sysu(data_path, mode=args.mode)
        gall_img, gall_label, gall_cam = process_gallery_sysu_multishot(data_path, mode=args.mode, trial=0)
        nquery = len(query_label)
        ngall = len(gall_label)
        print("Dataset statistics:")
        print("  ------------------------------")
        ...
        query_feat_pool, query_feat_fc = extract_query_feat(query_loader)
        for trial in range(10):
            gall_img, gall_label, gall_cam = process_gallery_sysu_multishot(data_path, mode=args.mode, trial=trial)
    ...
    for trial in range(10):
        test_trial = trial +1
        #model_path = checkpoint_path +  args.resume
        model_path = checkpoint_path + 'regdb_'+ args.method +'_p4_n8_lr_0.1_seed_0_trial_{}_best.t'.format(test_trial)
        if os.path.isfile(model_path):
    ...
    if flag == 1:
        ...
        print('Not enough checkpoints! The results are unreliable!')
